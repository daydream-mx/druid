...
impl<T: Data> Widget<T> for Flex<T> {
    ...
    fn layout(&mut self, ctx: &mut LayoutCtx, bc: &BoxConstraints, data: &T, env: &Env) -> Size {
        ...
        let use_baseline = matches!(self.direction, Axis::Horizontal) && matches!(self.cross_alignment, CrossAxisAlignment::Baseline);
        let minor = if use_baseline { max_below_baseline + max_above_baseline } else { minor };
